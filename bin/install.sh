...
find ./wp-content/plugins -mindepth 1 ! -regex '^./wp-content/plugins/db-watcher\(/.*\)?' -delete

wp --allow-root plugin activate db-watcher/mail-tools.php

# Install Test Suite
bash /var/www/html/wp-content/plugins/db-watcher/bin/install-wp-tests.sh wordpress_tests root root db latest true
